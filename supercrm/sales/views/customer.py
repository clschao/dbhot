...
class CutomerView(View):
    def get(self,request):
        ...
        # 全部客户
        if current_request_path == reverse('allcustomers'):
            tag = '3'
            customer_list = models.Customer.objects.all()

        # 公户
        elif current_request_path == reverse('customers'):
            tag = '1'
...
class TongJiView(View):
    ...
    def today(self):
        # today_date = datetime.datetime.today() 两个方式都可以
        today_date = datetime.datetime.now().date()
        customer_list = models.Customer.objects.filter(deal_date=today_date)
    ...
    def get(self,request):
        date = request.GET.get('type', 'today')
        if hasattr(self,date):
            ret = getattr(self,date)()
        s = ["哈哈","xx"]
        # s = "哈哈"
        return render(request,'saleshtml/customer_tongji.html',{'customer_list':ret,'s':s})
